...
module.exports = class Comment {
  ...
  static async searchComments(keyword) {
    ...
    SELECT * FROM comments WHERE comment ILIKE $1`, [`%${keyword}%`]);
    ...
    const getPostsOfComments = await Promise.all(uniquePostsIds.map(async (id) =>  await Post.getById(id)));
   
    const searchPostsResults = await pool.query(`
    SELECT * FROM posts WHERE title ILIKE $1 
    OR code ILIKE $1
    OR question ILIKE $1`, [`%${keyword}%`]);
